...
function statusBadgeClass(status: AbdmConfig["status"] | null) {
...
function statusLabel(status: AbdmConfig["status"] | null) {
...
export default function AbhaConfigPage() {
  ...
  const [callbackUrls, setCallbackUrls] = React.useState("");
  ...
  const [status, setStatus] = React.useState<AbdmConfig["status"]>("NOT_CONFIGURED");

  const fetchConfig = React.useCallback(
    async (wsId: string, env: AbdmEnvironment) => {
      setLoading(true);
      try {
        const config = await apiFetch<AbdmConfig | null>(
        ...
        if (config && config.id) {
          ...
          setCallbackUrls(config.callbackUrls ?? "");
          setEnableAbhaLinking(config.enableAbhaLinking ?? false);
          setEnableConsentFlow(config.enableConsentFlow ?? false);
          setEnableHealthRecords(config.enableHealthRecords ?? false);
  ...
  async function handleSave() {
    ...
    if (!clientId.trim()) {
      toast({ title: "Validation Error", description: "Client ID is required.", variant: "destructive" });
      return;
    }
    if (!clientSecret.trim()) {
      toast({ title: "Validation Error", description: "Client Secret is required.", variant: "destructive" });
      return;
    }
    ...
    try {
      const body = {
        ...
        callbackUrls: callbackUrls.trim(),
        enableAbhaLinking,
        enableConsentFlow,
        enableHealthRecords,
      };
      ...
      if (configId) {
        await apiFetch(`/api/compliance/abdm/config/${configId}`, { method: "PATCH", body });
        toast({ title: "Configuration Updated", description: "ABHA settings saved successfully." });
      } else {
        const created = await apiFetch<AbdmConfig>(`/api/compliance/abdm/config`, { method: "POST", body });
        setConfigId(created.id);
        setStatus(created.status ?? "CONFIGURED");
        toast({ title: "Configuration Created", description: "ABHA settings saved successfully." });
      }

      await fetchConfig(workspaceId, activeEnv);
    } catch (e: any) {
      toast({ title: "Save Failed", description: e.message ?? "Failed to save configuration.", variant: "destructive" });
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleTestConnection() {
    if (!configId) {
      toast({ title: "Cannot Test", description: "Save the configuration first before testing.", variant: "destructive" });
      return;
    }

    setTesting(true);
    try {
      await apiFetch(`/api/compliance/abdm/config/${configId}/test`, { method: "POST", body: {} });
      toast({ title: "Connection Successful", description: "ABDM API connection test passed." });
      setStatus("TESTED");
    } catch (e: any) {
      toast({ title: "Connection Failed", description: e.message ?? "ABDM API connection test failed.", variant: "destructive" });
    } finally {
      setTesting(false);
    }
  }
    ...
    <AppShell title="ABHA Configuration">
      ...
      <div className="grid gap-6">
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <div className="text-3xl font-semibold tracking-tight">ABHA Configuration</div>
              <div className="mt-1 text-sm text-zc-muted">
                Configure ABDM API credentials and feature toggles for ABHA integration.
              </div>
            </div>
          </div>
        ...
        {!activeBranchId ? (
          ...
          <Tabs value={activeEnv} onValueChange={(v) => setActiveEnv(v as AbdmEnvironment)}>
            <TabsList
              className={cn(
                ...
                <>
                    ...
                    <CardContent className="pt-4 space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-xs text-zc-muted">
                          Enter one URL per line. These URLs will receive ABDM callbacks.
                        </p>
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card className="overflow-hidden">
                    <CardHeader className="pb-3">
                      ...
                      <CardDescription>
                        Enable or disable specific ABDM features for this environment.
                      </CardDescription>
                    </CardHeader>
                    <Separator />
                    <CardContent className="pt-4 space-y-4">
                      <div className="flex items-center justify-between rounded-xl border border-zc-border bg-zc-panel/20 px-4 py-3">
                        <div>
                          <p className="text-sm font-medium text-zc-text">ABHA Linking</p>
                          <p className="text-xs text-zc-muted">
                            Allow patients to link their ABHA address to their profile.
                          </p>
                        </div>
                        <Switch checked={enableAbhaLinking} onCheckedChange={setEnableAbhaLinking} disabled={saving} />
                      </div>

                      <div className="flex items-center justify-between rounded-xl border border-zc-border bg-zc-panel/20 px-4 py-3">
                        <div>
                          <p className="text-sm font-medium text-zc-text">Consent Flow</p>
                          <p className="text-xs text-zc-muted">
                            Enable ABDM consent management for health data exchange.
                          </p>
                        </div>
                        <Switch checked={enableConsentFlow} onCheckedChange={setEnableConsentFlow} disabled={saving} />
                      </div>

                      <div className="flex items-center justify-between rounded-xl border border-zc-border bg-zc-panel/20 px-4 py-3">
                        <div>
                          <p className="text-sm font-medium text-zc-text">Health Records</p>
                          <p className="text-xs text-zc-muted">
                            Push and pull health records via ABDM Health Information Exchange.
                          </p>
                        </div>
                        <Switch checked={enableHealthRecords} onCheckedChange={setEnableHealthRecords} disabled={saving} />
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        variant="outline"
                        className="gap-2"
                        onClick={() => { if (workspaceId) fetchConfig(workspaceId, activeEnv); }}
