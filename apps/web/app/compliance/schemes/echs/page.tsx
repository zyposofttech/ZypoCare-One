...
export default function EchsPage() {
  ...
  const handleSave = async () => {
    ...
    if (!form.cityCategory) { toast({ title: "Validation", description: "City category is required for ECHS.", variant: "destructive" }); return; }
    setSaving(true);
    try {
  ...
  return (
    <AppShell title="ECHS Configuration">
      ...
      <div className="grid gap-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0"><div className="text-3xl font-semibold tracking-tight">ECHS Empanelment</div><div className="mt-1 text-sm text-zc-muted">Ex-Servicemen Contributory Health Scheme empanelment details and rate cards.</div></div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <CompliancePageHead pageId="compliance-schemes-echs" />
        ...
        {/* AI Insights */}
        <CompliancePageInsights pageId="compliance-schemes-echs" />
        ...
        {empLoading ? <div className="flex items-center justify-center py-12"><Loader2 className="h-6 w-6 animate-spin text-zc-muted" /></div> : (
          <Card className="overflow-hidden">
            <CardHeader className="pb-3"><div className="flex items-center justify-between"><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl border border-emerald-200 bg-emerald-50 dark:border-emerald-900/50 dark:bg-emerald-900/20"><FileText className="h-4 w-4 text-emerald-600 dark:text-emerald-400" /></span><div><CardTitle className="text-base">Empanelment Details</CardTitle><CardDescription>{empanelment ? `#${empanelment.empanelmentNumber}` : "No empanelment configured"}</CardDescription></div></div><Button variant="outline" size="sm" className="gap-1.5" onClick={() => setDialogOpen(true)}><Settings className="h-3.5 w-3.5" />{empanelment ? "Edit" : "Configure"}</Button></div></CardHeader>
            {empanelment && <><Separator /><CardContent className="pt-4"><div className="grid gap-3 md:grid-cols-3">
              <div className="rounded-xl border border-emerald-200 bg-emerald-50/50 p-3 dark:border-emerald-900/50 dark:bg-emerald-900/10"><div className="text-xs font-medium text-emerald-600 dark:text-emerald-400">Empanelment No.</div><div className="mt-1 text-lg font-bold text-emerald-700 dark:text-emerald-300">{empanelment.empanelmentNumber}</div></div>
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent className="max-w-md">
            <DialogHeader><DialogTitle>{empanelment ? "Edit" : "Configure"} ECHS Empanelment</DialogTitle><DialogDescription>Enter empanelment details for ECHS.</DialogDescription></DialogHeader>
            <Separator />
            <div className="space-y-4 pt-2">
              <div className="space-y-2"><Label htmlFor="empanelmentNumber">Empanelment Number *</Label><Input id="empanelmentNumber" value={form.empanelmentNumber} onChange={(e) => setForm((f) => ({ ...f, empanelmentNumber: e.target.value }))} placeholder="e.g. ECHS-POL-001" disabled={saving} /></div>
